...
import { getAgenciesByClientId, getClientById, getDashboardMetrics } from "@/services/mockData";
...
const InsightItem = ({
  ...
  return <div className="flex items-start gap-3 p-3 rounded-md bg-muted/50">
      ...
      <div className="flex-1">
        ...
        {(insight.source || insight.metric) && <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground flex-wrap">
            {insight.source && <span className="flex items-center">
                <Award className="h-3 w-3 mr-1" />
                {insight.source}
              </span>}
            {insight.metric && <span className="flex items-center">
                <Award className="h-3 w-3 mr-1" />
                {insight.metric}
              </span>}
            {insight.change !== undefined && <span className={`flex items-center ${insight.change > 0 ? 'text-emerald-500' : 'text-amber-500'}`}>
